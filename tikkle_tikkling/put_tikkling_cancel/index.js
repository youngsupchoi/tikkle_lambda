...
exports.put_tikkling_cancel = async (req, res) => {
  ...
  try {
    ...
    if (check_tikkling.length == 0) {
      console.log("비정상적 요청-put_tikkling_cancel: 티클링을 찾을 수 없습니다.");
      const return_body = {
        success: false,
        detail_code: "00",
    ...
    if (check_tikkling[0].tikkle_count == 0) {
    }

    //도착한 티클링 조각이 있는지 확인
    if (check_tikkling[0].tikkle_count != 0) {
      console.log("비정상적 요청-put_tikkling_cancel: 티클이 도착한 상태에서 티클링 취소를 요청");
      const return_body = {
        success: false,
        detail_code: "00",
      ...
      await Promise.all([
        queryDatabase(`UPDATE tikkling SET state_id = 2, terminated_at = now(), resolution_type = 'cancel' WHERE id = ?;`, [req.body.tikkling_id]),
        queryDatabase(`UPDATE users SET tikkling_ticket = tikkling_ticket + 1 WHERE id = ?;`, [id]),
        queryDatabase(`UPDATE products SET quantity = quantity + 1 WHERE id = (SELECT product_id FROM tikkling WHERE id = ?);`, [req.body.tikkling_id]),
      ]);
